...
function findMinMax([firstElement, ...restElements]) {
  ...
  let minItem = firstElement;
  let maxItem = firstElement;
  for (let i = 1; i < restElements.length; i++) {
    if (restElements[i] < minItem) {
      minItem = restElements[i];
    }
    if (restElements[i] > maxItem) {
      maxItem = restElements[i];
    }
  }
  console.log(`Min: ${minItem} Max: ${maxItem}`);
}
